...
func (s *Server) RegisterRoutes() http.Handler {
	...
	mux.HandleFunc("POST /v1/dm", directMessageHandler.CreateDirectMessage)
	mux.HandleFunc("/v1/ws/dm", directMessageHandler.HandleWebSocket)
	mux.HandleFunc("GET /v1/dms/{userId}", directMessageHandler.GetAllDirectMessages)
	mux.HandleFunc("GET /v1/dm", directMessageHandler.GetOneDirectMessageByParticipants)
	mux.HandleFunc("GET /v1/dm/{dmId}", directMessageHandler.GetOneDirectMessage)
	mux.HandleFunc("DELETE /v1/dm/{dmId}", directMessageHandler.DeleteDirectMessage)

	mux.HandleFunc("POST /v1/gc", groupChatHandler.CreateGroupChat)
	mux.HandleFunc("POST /v1/gc/members/{groupId}", groupChatHandler.AddMembers)
	mux.HandleFunc("/v1/ws/gc", groupChatHandler.HandleWebSocket)
	mux.HandleFunc("GET /v1/gcs/{userId}", groupChatHandler.GetAllGroupChats)
	mux.HandleFunc("GET /v1/gc/{groupId}", groupChatHandler.GetOneGroupChat)
	mux.HandleFunc("PATCH /v1/gc/{groupId}", groupChatHandler.UpdateGroupChat)
	mux.HandleFunc("DELETE /v1/gc/{groupId}/members/{userId}", groupChatHandler.LeaveGroupChat)
	mux.HandleFunc("DELETE /v1/gc/{groupId}", groupChatHandler.DeleteGroupChat)
